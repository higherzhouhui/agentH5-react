...
import { myCommissionSettleRecord } from '../../api/form.js'
...
const AnchorList = () => {
    ...
    async function loadMore(month, page) {
        ...
        let append = await myCommissionSettleRecord({
            month,
            pageNum: page_num,
            pageSize
        })
    ...
    const toDetail = (state) => {
        history('/commission_settlement_detail', { state })
    }
    const search = (month) => {
        setList([])
        setHasMore(false)
    ...
    return <div className={style.formBg}>
        <Header title={t('CUST.translate284')} topDom={<TimePicker onChange={search} />} />
        {
            list.length > 0 ?
                <InfiniteScroll
                    next={loadMore}
                    ...
                    <List className={style.formBody}>
                        {list.map((item, index) => {
                            return load ? <Skeleton key={index} style={{ height: '12rem' }} animated className={`${style.formItem} ${load && 'loading'}`}  /> :
